...
public class AdminController : Controller
{
    ...
    private static readonly TimeSpan AdminPostsCacheDuration = TimeSpan.FromSeconds(45);
    ...
    private const string FixedAuthorName = "Lê Thị Hồng";
    private static long _adminPostsCacheVersion = 1;
    ...
    public async Task<IActionResult> SaveProduct(AdminProductEditorViewModel editor)
    {
        ...
        if (!string.IsNullOrWhiteSpace(editor.SeoKeyword) && editor.SeoKeyword.Trim().Length > 255)
        {
            TempData["AdminProductsError"] = "Từ khóa SEO không được vượt quá 255 ký tự.";
            return RedirectToAction(nameof(Products), new { id = editor.Id });
        }
        ...
        product.SeoKeyword = NormalizeNullableText(editor.SeoKeyword?.Trim());
    ...
    {
        var successMessage = TempData[SuccessTempDataKey]?.ToString();
        var errorMessage = TempData[ErrorTempDataKey]?.ToString();

        var canUseCachedModel = !editId.HasValue
            && string.IsNullOrWhiteSpace(successMessage)
            && string.IsNullOrWhiteSpace(errorMessage);

        AdminPostsPageViewModel model;

        if (canUseCachedModel)
        {
            var cacheKey = BuildAdminPostsCacheKey(searchTerm, status, categoryId, page);
            model = await _memoryCache.GetOrCreateAsync(cacheKey, async entry =>
            {
                entry.AbsoluteExpirationRelativeToNow = AdminPostsCacheDuration;

                return await BuildAdminPostsPageViewModelAsync(
                    searchTerm,
                    status,
                    categoryId,
                    editId,
                    ...
                    errorMessage,
                    page: page);
            }) ?? new AdminPostsPageViewModel();
        }
        else
        {
            model = await BuildAdminPostsPageViewModelAsync(
                searchTerm,
                status,
                categoryId,
                editId,
    ...
    public async Task<IActionResult> SavePost([Bind(Prefix = EditorInputPrefix)] AdminPostEditorInputModel input, List<IFormFile>? imageFiles, List<IFormFile>? galleryFiles)
    {
        ...
        input.SeoKeyword = input.SeoKeyword?.Trim();
        input.Excerpt = input.Excerpt?.Trim() ?? string.Empty;
        input.Content = RemoveAuthorOverrideMarker(input.Content?.Trim() ?? string.Empty);
        ...
        input.AuthorMode = string.Equals(input.AuthorMode, "custom", StringComparison.OrdinalIgnoreCase) ? "custom" : "fixed";
        input.CustomAuthorName = input.CustomAuthorName?.Trim();
        ...
        if (input.AuthorMode == "custom" && string.IsNullOrWhiteSpace(input.CustomAuthorName))
        {
            ModelState.AddModelError($"{EditorInputPrefix}.{nameof(AdminPostEditorInputModel.CustomAuthorName)}", "Vui lòng nhập tên tác giả.");
        }
        ...
        try
        {
            ...
            post.SeoKeyword = NormalizeNullableText(input.SeoKeyword);
            post.Content = input.Content;
            var resolvedAuthorName = input.AuthorMode == "custom"
                ? input.CustomAuthorName!.Trim()
                : FixedAuthorName;

            post.CoverImage = NormalizeNullableText(input.CoverImage);
            post.AuthorId = null;
            post.Content = SetAuthorOverrideMarker(input.Content, resolvedAuthorName);
            ...
            await InvalidateBlogCachesAsync(post.Id);
    ...
    public async Task<IActionResult> UploadInlineBlogImage(IFormFile? file, string? alternativeDescription)
    {
        if (file is null || file.Length <= 0)
        {
            return Json(new { success = false, message = "Không có ảnh để tải lên." });
        ...
        var seoFileBaseName = BuildUniqueInlineBlogImageFileBaseName(inlineFolder, alternativeDescription);
        var useExactFileBaseName = !string.IsNullOrWhiteSpace(seoFileBaseName);
        var baseName = useExactFileBaseName
            ? seoFileBaseName!
            : BuildBlogImageFileBaseName(Path.GetFileNameWithoutExtension(file.FileName));
        var sequence = useExactFileBaseName ? 0 : GetNextAvailableImageSequence(inlineFolder, baseName);
    ...
    public async Task<IActionResult> DeletePost(int id, string? searchTerm, string? returnStatus, int? returnCategoryId, int? editId, int? returnPage)
    {
        ...
        try
        {
            ...
            await InvalidateBlogCachesAsync(id);
    ...
    public async Task<IActionResult> SaveCategory([Bind(Prefix = CategoryEditorInputPrefix)] AdminCategoryEditorInputModel input)
    {
        ...
        try
        {
            ...
            await _context.SaveChangesAsync();
            await InvalidateBlogCachesAsync();
    ...
    public async Task<IActionResult> SaveBanner([FromBody] AdminMarketingViewModel model)
    {
        ...
        await _context.SaveChangesAsync();
        await InvalidateBannerCachesAsync();
    ...
    public async Task<IActionResult> SaveContact([FromBody] AdminMarketingViewModel model)
    {
        ...
        await _context.SaveChangesAsync();
        await InvalidateContactCachesAsync();
    ...
    public async Task<IActionResult> SaveSeo([FromBody] AdminMarketingViewModel model)
    {
        ...
        await _context.SaveChangesAsync();
        await InvalidateSeoCachesAsync();
        ...
        int? page = null)
    {
        ...
        IQueryable<BlogPost> filteredPostsQuery = _context.BlogPosts
            .AsNoTracking();

        if (!string.IsNullOrWhiteSpace(normalizedSearchTerm))
        {
            var keywordPattern = $"%{normalizedSearchTerm}%";
            filteredPostsQuery = filteredPostsQuery.Where(post =>
                EF.Functions.Like(post.Title ?? string.Empty, keywordPattern)
                || EF.Functions.Like(post.Excerpt ?? string.Empty, keywordPattern)
                || EF.Functions.Like(post.Slug ?? string.Empty, keywordPattern));
        }

        if (!string.IsNullOrWhiteSpace(normalizedStatusFilter) && normalizedStatusFilter != "missing-cover")
        {
            filteredPostsQuery = filteredPostsQuery.Where(post => (post.Status ?? string.Empty).ToLower() == normalizedStatusFilter);
        }
        ...
        if (selectedCategoryId.HasValue)
        {
            var categoryPostIds = _context.BlogPostCategories
                .AsNoTracking()
                .Where(mapping => mapping.PostId.HasValue && mapping.CategoryId == selectedCategoryId.Value)
                .Select(mapping => mapping.PostId!.Value)
                .Distinct();

            filteredPostsQuery = filteredPostsQuery.Where(post => categoryPostIds.Contains(post.Id));
        }

        if (normalizedStatusFilter == "missing-cover")
        {
            filteredPostsQuery = filteredPostsQuery.Where(post => string.IsNullOrWhiteSpace(post.CoverImage));
        }

        var totalFilteredPosts = await filteredPostsQuery.CountAsync();
        ...
        var pagedPostRecords = await filteredPostsQuery
        ...
        AdminBlogPostRecord? editPostRecord = null;
        if (editId.HasValue)
        {
            editPostRecord = await _context.BlogPosts
                .AsNoTracking()
                .Where(post => post.Id == editId.Value)
                .Select(post => new AdminBlogPostRecord
                {
                    Id = post.Id,
                    Title = post.Title,
        ...
        var scopedPostIds = pagedPostRecords
            .Select(post => post.Id)
            .Concat(editPostRecord is not null ? [editPostRecord.Id] : [])
            .Distinct()
            .ToList();

        var categoryMappings = scopedPostIds.Count == 0
            ? []
            : await _context.BlogPostCategories
                ...
                .Where(mapping =>
                    ...
                    && scopedPostIds.Contains(mapping.PostId.Value))
                .Join(
        ...
        var productMappings = scopedPostIds.Count == 0
            ? []
        ...
        var galleryRecords = scopedPostIds.Count == 0
            ? []
            : await _context.BlogImages
                .AsNoTracking()
                .Where(image => image.BlogId.HasValue && scopedPostIds.Contains(image.BlogId.Value))
        ...
        var postRecordById = pagedPostRecords.ToDictionary(post => post.Id);
        if (editPostRecord is not null)
        {
            postRecordById[editPostRecord.Id] = editPostRecord;
        }
        ...
        var pagedPosts = pagedPostRecords
            .Select(post =>
            {
                ...
                var publicUrl = Url.RouteUrl("blog-detail-seo", new { slug = BuildBlogSlug(post.Slug, post.Id) }) ?? string.Empty;
                ...
                return new AdminPostRowViewModel
                {
                    ...
                    Author = ResolveEffectiveAuthorName(post.AuthorName, post.Content),
                    ...
                    Excerpt = BuildAdminExcerpt(post.Excerpt, null),
        ...
        var totalPosts = await _context.BlogPosts.AsNoTracking().CountAsync();
        var publishedCount = await _context.BlogPosts.AsNoTracking().CountAsync(post => (post.Status ?? string.Empty).ToLower() == "published");
        var draftCount = await _context.BlogPosts.AsNoTracking().CountAsync(post => (post.Status ?? string.Empty).ToLower() == "draft");
        var reviewCount = await _context.BlogPosts.AsNoTracking().CountAsync(post => (post.Status ?? string.Empty).ToLower() == "review");
        var hiddenCount = await _context.BlogPosts.AsNoTracking().CountAsync(post => (post.Status ?? string.Empty).ToLower() == "hidden");
        var missingCoverCount = await _context.BlogPosts.AsNoTracking().CountAsync(post => string.IsNullOrWhiteSpace(post.CoverImage));
        var categoryPostCounts = await _context.BlogPostCategories
            .AsNoTracking()
            .Where(mapping => mapping.PostId.HasValue && mapping.CategoryId.HasValue)
            .GroupBy(mapping => mapping.CategoryId!.Value)
            .Select(group => new { CategoryId = group.Key, Count = group.Select(mapping => mapping.PostId!.Value).Distinct().Count() })
            .ToDictionaryAsync(item => item.CategoryId, item => item.Count);
        var activeCategoryCount = categoryPostCounts.Count(item => item.Value > 0);
        var recentUpdatesCount = await _context.BlogPosts.AsNoTracking().CountAsync(post =>
            (post.UpdatedAt.HasValue && post.UpdatedAt.Value >= last7Days)
            || (post.CreatedAt.HasValue && post.CreatedAt.Value >= last7Days));
        var needsUpdateCount = await _context.BlogPosts.AsNoTracking().CountAsync(post =>
            (post.Status ?? string.Empty).ToLower() == "published"
            && post.UpdatedAt.HasValue
            && post.UpdatedAt.Value < refreshThreshold);
        var averageSeoScore = pagedPosts.Count == 0 ? 0 : (int)Math.Round(pagedPosts.Average(post => post.SeoScore));
        var totalGalleryImages = await _context.BlogImages.AsNoTracking().CountAsync();
        var totalRelationMappings = await _context.BlogPostProducts.AsNoTracking().CountAsync();
        ...
        IReadOnlyDictionary<int, List<AdminBlogImageRecord>> galleryByPostId)
    {
        ...
        else if (editId.HasValue && postRecordById.TryGetValue(editId.Value, out var post))
        {
            ...
            input = new AdminPostEditorInputModel
            {
                ...
                SeoKeyword = post.SeoKeyword?.Trim(),
                Excerpt = post.Excerpt?.Trim() ?? string.Empty,
                Content = post.Content?.Trim() ?? string.Empty,
                CoverImage = post.CoverImage?.Trim() ?? string.Empty,
                AuthorId = null,
                AuthorMode = ResolveAuthorMode(post.Content),
                CustomAuthorName = ResolveCustomAuthorForInput(post.Content),
    ...
    private async Task InvalidateBlogCachesAsync(int? postId = null)
    {
        Interlocked.Increment(ref _adminPostsCacheVersion);
        ...
        _memoryCache.Remove(SiteCacheKeys.HeaderServiceMenuItems);
        _memoryCache.Remove(SiteCacheKeys.FooterTopServicePosts);
        _memoryCache.Remove(SiteCacheKeys.RecentNewsPosts);
        _memoryCache.Remove(SiteCacheKeys.ScrapPurchaseServicePosts);
        ...
        await _outputCacheStore.EvictByTagAsync(SiteCacheKeys.BlogOutputTag, HttpContext.RequestAborted);
        await _outputCacheStore.EvictByTagAsync(SiteCacheKeys.HomeOutputTag, HttpContext.RequestAborted);
    }

    private async Task InvalidateProductCachesAsync()
    {
        _memoryCache.Remove(SiteCacheKeys.FooterFeaturedProducts);
        _memoryCache.Remove(SiteCacheKeys.HomeProducts);
        _memoryCache.Remove(SiteCacheKeys.ProductCategoryNames);

        await _outputCacheStore.EvictByTagAsync(SiteCacheKeys.ProductOutputTag, HttpContext.RequestAborted);
        await _outputCacheStore.EvictByTagAsync(SiteCacheKeys.HomeOutputTag, HttpContext.RequestAborted);
    }

    private async Task InvalidateBannerCachesAsync()
    {
        _memoryCache.Remove(SiteCacheKeys.ActiveBanners);

        await _outputCacheStore.EvictByTagAsync(SiteCacheKeys.HomeOutputTag, HttpContext.RequestAborted);
    }

    private async Task InvalidateContactCachesAsync()
    {
        _memoryCache.Remove(SiteCacheKeys.ContactInfo);
        _memoryCache.Remove(SiteCacheKeys.StructuredDataSettings);

        await _outputCacheStore.EvictByTagAsync(SiteCacheKeys.HomeOutputTag, HttpContext.RequestAborted);
    }

    private async Task InvalidateSeoCachesAsync()
    {
        _memoryCache.Remove(SiteCacheKeys.SeoSettings);
        _memoryCache.Remove(SiteCacheKeys.StructuredDataSettings);

        await _outputCacheStore.EvictByTagAsync(SiteCacheKeys.HomeOutputTag, HttpContext.RequestAborted);
    }

    private static string BuildAdminPostsCacheKey(string? searchTerm, string? status, int? categoryId, int? page)
    {
        var version = Interlocked.Read(ref _adminPostsCacheVersion);
        ...
        var normalizedCategoryId = categoryId > 0 ? categoryId.Value : 0;
        var normalizedPage = page > 0 ? page.Value : 1;
        ...
        if (string.IsNullOrWhiteSpace(normalizedSearchTerm)
            ...
            && normalizedPage == 1)
        {
            return $"admin-posts:v{version}:all";
        }

        return $"admin-posts:v{version}:search:{normalizedSearchTerm}|status:{normalizedStatus}|category:{normalizedCategoryId}|page:{normalizedPage}";
    }
    ...
    private static string? BuildUniqueInlineBlogImageFileBaseName(string uploadsDirectory, string? alternativeDescription)
    {
        var slug = BuildSeoFriendlySlug(alternativeDescription);
        ...
        while (Directory.EnumerateFiles(uploadsDirectory, $"{candidate}.*").Any())
        {
            candidate = $"{slug}-{duplicateIndex}";
            duplicateIndex++;
        }
    ...
    private sealed class AdminBlogPostRecord
    {
        ...
        public string? SeoKeyword { get; init; }
    ...
    private static string SetAuthorOverrideMarker(string content, string authorName)
    {
        var safeContent = RemoveAuthorOverrideMarker(content ?? string.Empty);
        var encodedAuthor = WebUtility.HtmlEncode(authorName.Trim());
        return $"<!--AUTHOR:{encodedAuthor}-->\n{safeContent}";
    }

    private static string RemoveAuthorOverrideMarker(string content)
    {
        ...
        return Regex.Replace(content, "^\\s*<!--AUTHOR:(.*?)-->\\s*", string.Empty, RegexOptions.Singleline).TrimStart();
    }

    private static string? ExtractAuthorOverride(string? content)
    {
        if (string.IsNullOrWhiteSpace(content))
        {
            return null;
    ...
    private static string ResolveEffectiveAuthorName(string? authorNameFromAdmin, string? content)
    {
        var overrideAuthor = ExtractAuthorOverride(content);
        if (!string.IsNullOrWhiteSpace(overrideAuthor))
        {
            return overrideAuthor;
        }

        return !string.IsNullOrWhiteSpace(authorNameFromAdmin) ? authorNameFromAdmin.Trim() : "Chưa gán tác giả";
    }

    private static string ResolveAuthorMode(string? content)
    {
        var overrideAuthor = ExtractAuthorOverride(content);
        if (string.IsNullOrWhiteSpace(overrideAuthor))
        {
        ...
        return string.Equals(overrideAuthor, FixedAuthorName, StringComparison.OrdinalIgnoreCase) ? "fixed" : "custom";
    }

    private static string ResolveCustomAuthorForInput(string? content)
    {
        var overrideAuthor = ExtractAuthorOverride(content);
        if (string.IsNullOrWhiteSpace(overrideAuthor) || string.Equals(overrideAuthor, FixedAuthorName, StringComparison.OrdinalIgnoreCase))
        {
            return string.Empty;
        }
        ...
        string? search)
    {
        ...
        var editor = selectedProduct is null
            ...
            : new AdminProductEditorViewModel
            {
                ...
                SeoKeyword = selectedProduct.SeoKeyword,
    ...
    private void DeletePhysicalFile(string? imagePath)
    {
        ...
        if (!ImagePathHelper.IsGeneratedVariantFile(physicalPath))
        {
            physicalPaths.Add(ImagePathHelper.BuildVariantPhysicalPath(physicalPath, SiteImageVariant.Original));
            physicalPaths.Add(ImagePathHelper.BuildVariantPhysicalPath(physicalPath, SiteImageVariant.Medium));
            physicalPaths.Add(ImagePathHelper.BuildVariantPhysicalPath(physicalPath, SiteImageVariant.Thumbnail));
        }
