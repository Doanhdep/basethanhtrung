...
public sealed class BlogImageProcessor : IBlogImageProcessor
{
    public async Task<IReadOnlyList<ProcessedBlogImageResult>> ProcessAndSaveAsync(
        ...
        bool useExactFileBaseName = false)
    {
        ...
        for (var i = 0; i < imageFiles.Count; i++)
        {
            ...
            var baseName = useExactFileBaseName && imageFiles.Count == 1
                ? fileBaseName
                : $"{fileBaseName}{seq}";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            using var original = image.Clone(ctx =>
                ResizeIfLarger(ctx, image.Width, image.Height, new ResizeOptions { Mode = ResizeMode.Max, Size = new Size(1920, 1920), Sampler = KnownResamplers.Lanczos3 }));
        ...
        {
            using var medium = image.Clone(ctx =>
                ResizeIfLarger(ctx, image.Width, image.Height, new ResizeOptions { Mode = ResizeMode.Max, Size = new Size(1280, 1280), Sampler = KnownResamplers.Lanczos3 }));
            await medium.SaveAsWebpAsync(mediumPath, webpEncoderMedium, cancellationToken);
        }
        ...
        {
            using var thumb = image.Clone(ctx =>
                ResizeIfLarger(ctx, image.Width, image.Height, new ResizeOptions { Mode = ResizeMode.Crop, Size = new Size(640, 480), Position = AnchorPositionMode.Center, Sampler = KnownResamplers.Lanczos3 }));
            await thumb.SaveAsWebpAsync(thumbPath, webpEncoderThumb, cancellationToken);
        }
