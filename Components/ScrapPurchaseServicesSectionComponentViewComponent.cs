...
namespace WebThuMuaPheLieu.Components
{
    ...
    {
        private static readonly TimeSpan ServicePostsCacheDuration = TimeSpan.FromMinutes(5);

        private readonly AppDbContext _context;
        private readonly IMemoryCache _memoryCache;
        private readonly IBlogImageProcessor _blogImageProcessor;
        private readonly ILogger<ScrapPurchaseServicesSectionComponentViewComponent> _logger;

        public ScrapPurchaseServicesSectionComponentViewComponent(
            AppDbContext context,
            IMemoryCache memoryCache,
            IBlogImageProcessor blogImageProcessor,
            ILogger<ScrapPurchaseServicesSectionComponentViewComponent> logger)
        {
            _context = context;
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            var posts = await _memoryCache.GetOrCreateAsync(SiteCacheKeys.ScrapPurchaseServicePosts, async entry =>
            {
                fromCache = false;
                entry.AbsoluteExpirationRelativeToNow = ServicePostsCacheDuration;
            ...
            stopwatch.Stop();
            _logger.LogDebug(
                "ScrapPurchaseServicesSectionComponent: posts={PostCount}, cache={CacheHit}, elapsed={ElapsedMs}ms",
                posts.Count,
                fromCache,
